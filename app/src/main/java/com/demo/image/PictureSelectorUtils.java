...
class PictureSelectorUtils {
    private static volatile PictureSelectorUtils pictureSelectorUtils;
    private PictureSelector pictureSelector;
    private final PictureSelectionModel pictureSelectionModel;
    ...
    public static PictureSelectorUtils getInstance(Activity activity) {
        if (pictureSelectorUtils == null) {
            synchronized (PictureSelectorUtils.class) {
                if (pictureSelectorUtils == null) {
                    pictureSelectorUtils = new PictureSelectorUtils(activity);
                }
            }
        }
    ...
    private PictureSelectorUtils(Activity activity) {
        ...
        pictureSelectionModel = new PictureSelectionModel(pictureSelector, PictureMimeType.ofAll());

    }

    public PictureSelectorUtils configSelector(int mediaType, boolean isCamera) {
        if (pictureSelector == null || pictureSelectionModel == null) {
            return null;
        }
        pictureSelector.openGallery(mediaType);
        pictureSelectionModel.imageEngine(GlideEngine.createGlideEngine());
        pictureSelectionModel.selectionMode(PictureConfig.MULTIPLE);
        pictureSelectionModel.isCamera(isCamera);
        pictureSelectionModel.imageFormat(PictureMimeType.JPEG);
        pictureSelectionModel.setCameraImageFormat(PictureMimeType.JPEG);
        pictureSelectionModel.maxSelectNum(4);
        pictureSelectionModel.minSelectNum(0);
        ...
        pictureSelectionModel.isOriginalImageControl(true);
